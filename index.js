...
clientDIS.on('messageReactionAdd', (reaction, user) => {
    if(reaction.message.channel.id == '685192557226885155' && reaction.emoji.name === '✅') {
        ...
        members.fetch(user).then(result => result.roles.add('712001337440862269')).catch(console.error);
    }
})
...
clientTWI.on("cheer", (channel, userstate, message) => {
    if(userstate.bits < 100) {
        ...
        clientDIS.channels.cache.get(chitchatChannel).send("<:Red_Love:703246776781635674> <:Red_Love:703246776781635674> " + userstate.name + " has cheered " + userstate.bits + " during Redfur's stream <:Red_Love:703246776781635674> <:Red_Love:703246776781635674>");
    }
});
