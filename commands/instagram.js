...
module.exports = {
	...
	async execute(client, message, args) {
		...
		channel.send(instagramEmbed);

		if (args.length != 1) {
			return message.channel.send(`https://www.instagram.com/sweetycomics/`); 
		}
		else {
			try {
				const match = await SocialMedia.findOne({where: {platform: `instagram`, username: args[0]}});
				if(match) {
