...
module.exports = {
	...
	async execute(client, message, args) {
		...
		channel.send(twitchEmbed);

		if (args.length != 1) {
			return message.channel.send(`https://www.twitch.tv/redfur_13`); 
		}
		else {
			try {
				const match = await SocialMedia.findOne({where: {platform: `twitch`, username: args[0]}});
				if(match) {
