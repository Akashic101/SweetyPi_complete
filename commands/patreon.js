...
module.exports = {
	...
	async execute(client, message, args) {
        ...
        channel.send(patreonEmbed);

        if (args.length != 1) {
            return message.channel.send("https://www.patreon.com/Redfur13") 
        }
        else {
            try {
                const match = await SocialMedia.findOne({where: {platform: "patreon", username: args[0]}});
                if(match) {
