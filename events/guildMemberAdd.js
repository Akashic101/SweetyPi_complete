...
module.exports = async (client, member) => {
	...
	ctx.fillStyle = `red`;
	ctx.fillText(`Member joined`, canvas.width / 4, canvas.height / 5);
	ctx.fillStyle = `white`;
	...
	ctx.font = `18px Arial`;
	ctx.fillText(`${new Date().toUTCString()}`, 250, 215);
	...
	const finalImage = new Discord.MessageAttachment(canvas.toBuffer(), `memberJoined.png`);
